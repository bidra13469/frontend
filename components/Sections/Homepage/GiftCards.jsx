      ...
      <div className="w-full pt-[60px]">
        <div className="theme-container mx-auto">
          <div className="w-full">
            <div className="title flex justify-between items-center gap-2 flex-wrap w-full">
              <div className="mb-3 sm:mb-0">
                <p className="md:text-lg text-sm font-medium text-primary-blue">
                  <StringLang string="Unlimited Offer" />
                </p>
              ...
              <div>
                <Link href={`/products?category=${datas?.category?.slug}`}>
                  <div className="lg:py-4 lg:px-[25px] py-3 px-5 inline-flex rtl:space-x-reverse space-x-2.5 items-center bg-primary-blue text-primary-black hover:bg-white hover:text-black  group common-transition rounded-[5px]">
            ...
            <div className="grid lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 lg:gap-[30px] gap-2.5 mt-[30px] w-full">
              {datas?.products?.map((product, i) => (
                <Link key={i} href={`/products/${product?.slug}`}>
                  <div className="border border-[#23262B] rounded-md hover:border-primary-blue p-[6px] common-transition h-[170px]">
                    <img
                      src={process.env.BASE_URL + product?.thumbnail_image}
                      alt=""
