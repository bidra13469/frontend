“use client”;import React, { useEffect, useState } from “react”;import Link from “next/link”;import { useSearchParams, useRouter, usePathname } from “next/navigation”;import { toast } from “react-toastify”;import { useLazyLogoutQuery } from “@/store/features/auth/apiSlice”;import { useLazyGetCartQuery } from “@/store/features/cart/apiSlice”;import { useSelector } from “react-redux”;import StringLang from “@/utilities/StringLang”;import DropDown from “../Helper/DropDown”;import { useDispatch } from “react-redux”;import { changeCurrency } from “@/store/features/setup/setupSlice”;import { getCookie, hasCookie, setCookie } from “cookies-next”;

function AppHeader({ settings, categories, languages, currencies }) {const authProfile = useSelector((state) => state.auth.user);const router = useRouter();const searchParams = useSearchParams();const verified = searchParams.get(“verified”);const seller_approval = searchParams.get(“seller_approval”);const message = searchParams.get(“message”);const pathname = usePathname();const [logout, { isLoading, isSuccess }] = useLazyLogoutQuery();useEffect(() => {if (!!verified) {router.push(”/”);}if (verified === “yes”) {toast.success(message);}if (verified === “no”) {toast.error(message);}}, [verified]);useEffect(() => {if (!!seller_approval) {router.push(”/”);}if (seller_approval === “yes”) {logout();toast.success(message);}if (seller_approval === “no”) {toast.error(message);}}, [seller_approval]);const [searchKey, setSearchkey] = useState(””);const [open, setOpen] = useState(false);const [tab, setTab] = useState(“category”);const [categoryToggle, setCategoryToggle] = useState(false);useEffect(() => {setCategoryToggle(false);}, [pathname, searchParams]);const auth = useSelector((state) => state.auth.accessToken);const [getCartData, { data }] = useLazyGetCartQuery();

useEffect(() => {if (!!auth) {getCartData();}}, [auth]);// currencyconst dispatch = useDispatch();const changeCurrencyHandler = (value) => {dispatch(changeCurrency(value));};// translateconst [selectedLanguage, setLanguage] = useState(languages && languages.length > 0 ? languages[0] : null);useEffect(() => {let addScript = document.createElement(“script”);addScript.setAttribute(“src”,“//translate.google.com/translate_a/element.js?cb=googleTranslateElementInit”);document.body.appendChild(addScript);window.googleTranslateElementInit = googleTranslateElementInit;}, []);

const googleTranslateElementInit = () => {new window.google.translate.TranslateElement({pageLanguage: “auto”,autoDisplay: false,},“google_translate_element”);};const setCookieHandlerAction = (lang_code) => {// Get the current hostnameconst currentDomain = window.location.hostname;// Determine if the current domain is a subdomainconst isSubdomain = currentDomain.split(”.”).length >= 2;const cookieDomain = isSubdomain ? .${currentDomain} : currentDomain;// Set the cookiesetCookie(“googtrans”, /auto/${lang_code}, {path: “/”,domain: ${cookieDomain}, // Use the appropriate domainsecure: false,});if (currentDomain !== “localhost”) {deleteCookie(“googtrans”, ${currentDomain});if (currentDomain.split(”.”).length === 3) {const dotDomain = currentDomain.split(”.”).slice(1, 3).join(”.”);setCookie(“googtrans”, /auto/${lang_code}, {path: “/”,domain: ${dotDomain}, // Use the appropriate domainsecure: false,});}}};

useEffect(() => {if (languages && languages.length > 0) {if (hasCookie(“googtrans”)) {const getCode = getCookie(“googtrans”).replace(”/auto/”, “”);const findItem = languages.find((item) => item.lang_code === getCode);setLanguage(findItem);if (getCode === “ar” || getCode === “he”) {document.body.setAttribute(“dir”, “rtl”);} else {document.body.setAttribute(“dir”, “ltr”);}} else {setLanguage(languages[0]);if (languages[0].lang_code === “ar” ||languages[0].lang_code === “he”) {document.body.setAttribute(“dir”, “rtl”);} else {document.body.setAttribute(“dir”, “ltr”);}}}}, [languages]);

const langChange = (value) => {setCookieHandlerAction(value.lang_code);setLanguage(value);setTimeout(() => {window.location.reload();}, 2000);};return (<><imgsrc={process.env.BASE_URL + settings.logo}alt=””className=“w-[133px] object-contain h-[38px]”/><inputvalue={searchKey}onChange={(e) => setSearchkey(e.target.value)}placeholder=“Search your products…”className=“h-[52px] rounded ltr:pl-[22px] ltr:pr-[100px] rtl:pr-[22px] rtl:pl-[100px]  bg-[#0B0E12] border border-[#23262B] focus:outline-0 text-white  w-full placeholder:text-sm”type=“text”/><Link href={/products?search=${searchKey}}>
